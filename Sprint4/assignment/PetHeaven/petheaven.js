...
function fetchSpeakerData(file) {
    $.getJSON(file + ".json", function(data) {
      ...
      data.owners.forEach(function(owner) {
        var catbreed = $("<h1>").text(owner.catbreed);
        var image = $("<img>").attr("src", owner.image);
        var month = $("<h2>").html(owners.month + "<br>" + speaker.speaker);
        var description = $("<p>").text(speaker.text);

        $("main").append(catbreed, image, month, description);
      });
    });
  }
